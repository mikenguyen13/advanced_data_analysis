...
The `summary()` output ranks the variables by relative influence, shown in Figure @fig-boosting-boston-relinf. Here `rm` (average number of rooms) and `lstat` (percent lower-status population) dominate, which matches the intuition that house size and neighborhood drive price.
...
Figure @fig-boosting-gbm-perf-slow shows the loss falling as trees are added, with the optimal point marked. Because the learning rate is so small, the curve is still descending after many thousands of trees: a small learning rate can demand a great many trees.
...
Figure @fig-boosting-gbm-perf-fast shows that the faster, deeper model reaches a noticeably lower cross-validated RMSE with far fewer trees. Tuning one parameter at a time like this is instructive, but in practice we search the parameter space jointly with a grid search.
...
The first question is usually: which variable has the largest influence on the response? The `summary()` method answers this by relative influence, as plotted in Figure @fig-boosting-gbm-final-relinf.
...
The `vip` package produces a similar ranking with a cleaner default plot, shown in Figure @fig-boosting-gbm-vip.
...
The PDP in Figure @fig-boosting-gbm-pdp shows how predicted sale price rises with above-grade living area.
...
Figure @fig-boosting-gbm-ice draws the ICE curves, both in their raw form and centered so every curve starts at the same point, which makes differences in slope easier to compare.
...
Each bar in Figure @fig-boosting-gbm-lime shows how much a feature pushed that specific prediction up or down, which is exactly the kind of explanation you would want to give a stakeholder.
...
In Figure @fig-boosting-xgb-overfit, the red (training) curve keeps falling while the blue (test) curve flattens and then turns up: a textbook picture of overfitting as trees accumulate. Rather than guessing the right number of rounds, we can let XGBoost stop on its own once the test error stops improving, using `early_stopping_rounds`. Figure @fig-boosting-xgb-earlystop shows the result.
...
Gain is usually the most informative, so Figure @fig-boosting-xgb-importance plots the top ten features by gain.
...
Local explanations work the same way, except the observations must be one-hot encoded with the same treatment plan before they are explained. Figure @fig-boosting-xgb-lime shows the resulting explanations.
...
To make this concrete, Figure @fig-boosting-boston-pdp shows the PDPs for the two most important variables from our `gbm` Boston model, giving each variable's marginal effect on the response after integrating out the others.
...
The plots in Figure @fig-boosting-boston-pdp confirm the expected directions: predicted median house price rises with the average number of rooms (`rm`) and falls as the percentage of lower-status population (`lstat`) increases.
