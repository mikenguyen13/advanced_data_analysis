...
Table \@ref(tab:parallel-computing-backends) summarizes the two backends.
...
Table: (\#tab:parallel-computing-backends) Comparison of the FORK and PSOCK parallel backends in R, showing where each is available, whether it works across machines, and how workers obtain their environment.
...
Figure \@ref(fig:parallel-computing-multidplyr-workflow) sketches the idea: a data frame is split into groups, each group is shipped to a separate worker, the same `dplyr` operation runs on every worker at once, and the pieces are gathered back into one result.

```{r parallel-computing-multidplyr-workflow, echo=FALSE, fig.cap="The multidplyr workflow: a data frame is partitioned across workers, the same dplyr operation runs on each piece in parallel, and the per-worker results are collected back into a single data frame. Figure by Business Science."}
knitr::include_graphics("images/multidplyr.png")
```
