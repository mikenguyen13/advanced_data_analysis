...
Table @tbl-parallel-computing-backends summarizes the two backends.
...
Figure @fig-parallel-computing-multidplyr-workflow sketches the idea: a data frame is split into groups, each group is shipped to a separate worker, the same `dplyr` operation runs on every worker at once, and the pieces are gathered back into one result.

```{r fig-parallel-computing-multidplyr-workflow, echo=FALSE, fig.cap="The multidplyr workflow: a data frame is partitioned across workers, the same dplyr operation runs on each piece in parallel, and the per-worker results are collected back into a single data frame. Figure by Business Science."}
