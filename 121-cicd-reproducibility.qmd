# CI/CD and Reproducibility for ML {#sec-cicd-reproducibility}
...
2. Models decay even when code does not. Data drifts, so a model that passed its threshold last month may fail it now. A retraining pipeline that re-runs the same evaluation gate catches decay automatically, complementing the dedicated drift surveillance covered in the model monitoring chapter (@sec-model-monitoring).
