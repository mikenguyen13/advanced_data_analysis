...
Table @tbl-mixture-of-experts-dense-vs-sparse summarizes the trade-off between the two flavors so you can pick the right one for the job.
...
It helps to place MoE next to the other ensemble methods in this book. Table @tbl-mixture-of-experts-vs-ensembles lays out the comparison, and the single column that sets MoE apart is the last one: whether the combination weights are allowed to depend on the input.
...
We can also check that the gate behaves like a learned soft router: the probability of expert 2 should rise as $x$ increases. Figure @fig-mixture-of-experts-gate plots that learned gating probability across the input range.
