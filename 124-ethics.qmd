...
- Algorithmic Impact Assessments. A structured review, carried out before deployment, that documents who a system affects, what could go wrong, and how those risks will be monitored. Treat it the way an engineer treats a safety review: a deliberate pause to ask "what if this is wrong?" while changes are still cheap to make. A model card (@sec-model-card) is a natural companion artifact for recording the outcome of such a review.
...
- LIME. A method for explaining individual predictions in human-readable terms, covered in detail in the interpretable machine learning chapter (@sec-interpretable-machine-learning). When you can see *why* a model made a particular call, biased reasoning becomes much easier to spot.
