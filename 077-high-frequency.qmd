# High Frequency Predictors {#sec-high-frequency}
...
-   Feed the raw series to a sequence model such as a recurrent neural network (RNN) or a one-dimensional convolutional neural network (1D CNN), which learn their own representation (see the neural networks chapter, @sec-neural-networks).
...
which is exactly the form of ordinary least squares estimates whenever $(\mathbf{\Phi' \Phi})^{-1}$ is defined. In other words, we are regressing the data onto the Fourier basis functions, just as we did in the spline setting (@sec-spline).
...
-   Apply PCA (or another empirical dimension-reduction method, @sec-dimension-reduction) to the spectral coefficients across many series, keeping only the leading components.
...
The second, more general, approach is to treat the spectrogram as a plain image and apply the same machinery you would use on any image: reduce it with PCA or another method to get a feature vector, or feed it directly to a convolutional neural network (@sec-neural-networks) that learns its own features. This is why audio classification is so often done with image models: the spectrogram converts a hearing problem into a seeing problem.
