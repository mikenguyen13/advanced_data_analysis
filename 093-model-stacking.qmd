...
```{r modtuning-setup, cache=FALSE}
# This chapter's code is illustrative (reference snippets), so it is shown but
# not executed. cache=FALSE ensures this eval=FALSE setting is applied on every
# build, even when later chunks are cache hits.
library("finetune")
