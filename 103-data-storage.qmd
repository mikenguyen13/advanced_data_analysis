...
Table @tbl-data-storage-structured-vs-unstructured summarizes the main differences. Read it as a quick reference; the sections that follow unpack each row in words.
...
Figure @fig-data-storage-mongod-startup shows a typical `mongod` startup command together with these options in a terminal.

```{r fig-data-storage-mongod-startup, echo=FALSE, fig.cap="A terminal session launching the mongod server with the common startup options that control the port, data path, logfile, and forked background operation."}
