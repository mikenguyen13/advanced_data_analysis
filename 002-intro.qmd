...
For readers who like a visual map of how the methods relate, @fig-intro-method-map lays out the field at a glance, and the overview chapter that follows (@sec-overview) expands each branch. Two textbooks are the standard references and are cited throughout this book: @james2013 is the most accessible introduction, while @hastie2001a is the most mathematically complete.
...
@tbl-intro-method-families organizes the major method families by two questions: is the output (or the structure of interest) continuous or categorical, and is the problem supervised or unsupervised. Use it as a map; each entry is a chapter or section later in the book.
...
: Major method families, organized by output type (continuous or categorical) and by whether the problem is supervised or unsupervised. {#tbl-intro-method-families}
...
This choice exposes a tradeoff between prediction accuracy and model interpretability, summarized in @tbl-intro-restrictive-flexible. A restrictive model is easier to interpret and better suited to inference, while a flexible model usually predicts better but is harder to explain.
...
A concrete example of such an approximation is K-nearest neighbors (KNN), treated in full in @sec-knn, which estimates the conditional probability by simply looking at the labels of the $K$ closest training points:
...
> **Warning:** In high dimensions there is no such thing as a local neighborhood: every point is far from every other point. This is why methods built on local averaging (KNN, kernel smoothers; see @sec-kernel) degrade as $p$ grows, and why dimension reduction and regularization, covered later in this chapter, become necessary rather than optional.
...
Whether the model fits well depends heavily on how the sample size $n$ compares to the number of predictors $p$. The three regimes in @tbl-intro-n-vs-p recur throughout the book and explain why "too many predictors" is a real danger:
