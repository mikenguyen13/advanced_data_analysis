...
3.  Ensembling with specialization. Unlike bagging or stacking where every base model sees every input, MoE weights are input-dependent. The combination is local, not global, which is closer to a learned partition of the feature space than to a uniform average.

The core distinction from a standard ensemble is that the mixture weights depend on $x$. A stacked ensemble (see \@ref(model-stacking)) learns fixed blend weights; a mixture of experts learns weights $g_k(x)$ that change with the input.
...
Table \@ref(tab:mixture-of-experts-dense-vs-sparse) summarizes the trade-off between the two flavors so you can pick the right one for the job.
...
Table: (\#tab:mixture-of-experts-dense-vs-sparse) Comparison of dense and sparse top-$k$ mixtures of experts across compute cost, scale, training stability, and routing.

## MoE compared to related methods

It helps to place MoE next to the other ensemble methods in this book. Table \@ref(tab:mixture-of-experts-vs-ensembles) lays out the comparison, and the single column that sets MoE apart is the last one: whether the combination weights are allowed to depend on the input.
...
Table: (\#tab:mixture-of-experts-vs-ensembles) How a mixture of experts compares to bagging, stacking, and boosting, with input-dependent combination weights as the distinguishing feature.
...
Read the output as two columns, one per expert. One expert recovered a slope near $-1.6$ and the other near $+1.6$, which are exactly the two slopes we built into the simulation, even though the fit was never told where the regime boundary was. The gate learned a soft boundary near $x = 0$ on its own. Figure \@ref(fig:mixture-of-experts-fit) plots the data colored by which expert claims responsibility, overlays each expert's fitted line, and shows the mixture prediction.

```{r mixture-of-experts-fit, fig.cap="Two-expert mixture on piecewise data. Points are colored by the expert with higher responsibility, the dashed lines are the two linear experts, and the solid line is the gate-weighted mixture prediction.", fig.width=7, fig.height=5}
...
We can also check that the gate behaves like a learned soft router: the probability of expert 2 should rise as $x$ increases. Figure \@ref(fig:mixture-of-experts-gate) plots that learned gating probability across the input range.

```{r mixture-of-experts-gate, fig.cap="Learned gating probability for expert 2 as a function of x. The softmax gate forms a soft routing boundary near the regime change.", fig.width=7, fig.height=4}
...
-   If the relationship is globally smooth, a GAM (see \@ref(generalized-additive-models)) or a gradient-boosted tree (see \@ref(boosting)) is simpler and usually stronger.
