...
> **Key idea:** Boosting combines many "weak" learners (models that are only slightly better than guessing) into one strong "committee," by training each new learner to fix the mistakes of the current ensemble.
...
-   Because it relentlessly drives down training error, a gradient boosting machine (GBM) can overemphasize outliers and overfit. Cross-validation (Chapter \@ref(cross-validation)) is the main defense.

-   It can be computationally expensive, especially during a large grid search over hyperparameters.

Underneath all of this, the general framework of boosting is a stagewise additive model built from $B$ regression trees, which connects it directly to the additive models of Chapter \@ref(generalized-additive-models):
...
> **Intuition:** If the loss is a hill you want to walk down, the gradient points uphill. Boosting reads off the (negative) gradient at the current model, fits a tree to it, and takes a small downhill step scaled by the learning rate.
...
-   Subsampling: training each tree on a random subset of the data adds randomness that can reduce overfitting; tune it with cross-validation (Chapter \@ref(cross-validation)).
...
1.  $B$: the number of trees. Boosting *can* overfit if $B$ is too large, so we use cross-validation (Chapter \@ref(cross-validation)) to select $B$.
...
> **Note:** The inverse relationship between $\lambda$ and $B$ is the single most important thing to internalize. Halving the learning rate roughly doubles the number of trees you need. Slow learners are more accurate but more expensive.
...
> **Intuition:** Boosting is like a study group where, after every quiz, you spend more time on the questions the group keeps getting wrong, and you trust the answers of whoever has been most reliable so far.

<br>

### Why boosting works so well

It is reasonable to ask *why* this procedure should produce such accurate predictors. The cleanest explanation comes from viewing boosting as a basis function expansion (Chapter \@ref(basis-function-representations)):
...
> **Note:** AdaBoost did not start from "let's minimize exponential loss." It was designed first and only later understood as stagewise minimization of that loss. This after-the-fact view is what opened the door to generalizing it.
...
> **Intuition:** Scaling each tree by the learning rate means taking a small, deliberate step in the right direction rather than a giant leap. Many small steps reach a better place than a few large ones.
...
```{r boosting-boston-relinf, message=FALSE, warning=FALSE, fig.cap="Relative influence of each predictor in the gbm boosted model for the Boston housing data, ranked from most to least important."}
library(gbm)
library(ISLR2)
library(tree)
...
The `summary()` output ranks the variables by relative influence, shown in Figure \@ref(fig:boosting-boston-relinf). Here `rm` (average number of rooms) and `lstat` (percent lower-status population) dominate, which matches the intuition that house size and neighborhood drive price.
...
```{r boosting-gbm-perf-slow, fig.cap="Cross-validated loss as a function of the number of trees for the slow-learning gbm fit on the Ames data, with the optimal number of trees marked by the dashed line."}
...
Figure \@ref(fig:boosting-gbm-perf-slow) shows the loss falling as trees are added, with the optimal point marked. Because the learning rate is so small, the curve is still descending after many thousands of trees: a small learning rate can demand a great many trees.
...
```{r boosting-gbm-perf-fast, fig.cap="Cross-validated loss versus number of trees for the faster, deeper gbm fit (shrinkage 0.1, interaction depth 3) on the Ames data."}
# for reproducibility
set.seed(123)
...
Figure \@ref(fig:boosting-gbm-perf-fast) shows that the faster, deeper model reaches a noticeably lower cross-validated RMSE with far fewer trees. Tuning one parameter at a time like this is instructive, but in practice we search the parameter space jointly with a grid search.
...
A trained boosting model is accurate but, on its own, opaque. The next few tools turn it back into something a human can reason about: which variables matter, how each variable affects the prediction, and why a particular prediction came out the way it did.

##### Variable importance

The first question is usually: which variable has the largest influence on the response? The `summary()` method answers this by relative influence, as plotted in Figure \@ref(fig:boosting-gbm-final-relinf).

```{r boosting-gbm-final-relinf, fig.cap="Relative influence of the top predictors in the final tuned gbm model for the Ames housing data."}
par(mar = c(5, 8, 1, 1))
summary(
  gbm.fit.final,
  cBars = 10,
...
The `vip` package produces a similar ranking with a cleaner default plot, shown in Figure \@ref(fig:boosting-gbm-vip).

```{r boosting-gbm-vip, fig.cap="Variable importance for the final tuned gbm model, produced by the vip package."}
...
The PDP in Figure \@ref(fig:boosting-gbm-pdp) shows how predicted sale price rises with above-grade living area.

```{r boosting-gbm-pdp, fig.cap="Partial dependence of predicted sale price on above-grade living area (Gr_Liv_Area) for the final gbm model."}
# PDP
gbm.fit.final %>%
...
Figure \@ref(fig:boosting-gbm-ice) draws the ICE curves, both in their raw form and centered so every curve starts at the same point, which makes differences in slope easier to compare.

```{r boosting-gbm-ice, warning=FALSE, fig.cap="Individual conditional expectation curves for above-grade living area, shown non-centered (left) and centered (right) for the final gbm model."}
ice1 <- gbm.fit.final %>%
  partial(
...
> **Tip:** Below we call `lime::lime()` and `lime::explain()` with their package prefixes. The name `explain` is also defined by `dplyr`, so prefixing avoids a masking conflict that would otherwise break the call when both packages are loaded.

```{r boosting-gbm-lime, warning=FALSE, fig.cap="LIME local explanations for two test observations under the final gbm model, showing the features that most influenced each individual prediction."}
...
Each bar in Figure \@ref(fig:boosting-gbm-lime) shows how much a feature pushed that specific prediction up or down, which is exactly the kind of explanation you would want to give a stakeholder.
...
So far each tree has seen all of the data. Stochastic gradient boosting borrows the randomness that made bagging and random forests (Chapter \@ref(random-forests)) effective: by fitting each tree to a random subsample, we reduce the correlation between successive trees in the boosting sequence, which often improves generalization.
...
> **Tip:** Setting the row subsample fraction below 1 (the `bag.fraction` argument in `gbm`) is a cheap and reliable regularizer. It is one of the first knobs to try when a boosted model overfits.

## Penalized Gradient Boosting

Another way to control complexity is to penalize the tree parameters directly. L1 regularization (Lasso, Chapter \@ref(lasso-regression)) and L2 regularization (Ridge, Chapter \@ref(ridge-regression)) can both be imposed on the weights inside the trees, shrinking them toward zero and discouraging overly elaborate fits. This is part of what the "regularized boosting" in XGBoost refers to.
...
```{r boosting-xgb-overfit, fig.cap="Training (red) and cross-validated test (blue) RMSE by boosting round for the default XGBoost fit, illustrating overfitting as the number of trees grows."}
...
In Figure \@ref(fig:boosting-xgb-overfit), the red (training) curve keeps falling while the blue (test) curve flattens and then turns up: a textbook picture of overfitting as trees accumulate. Rather than guessing the right number of rounds, we can let XGBoost stop on its own once the test error stops improving, using `early_stopping_rounds`. Figure \@ref(fig:boosting-xgb-earlystop) shows the result.

```{r boosting-xgb-earlystop, fig.cap="Training (red) and cross-validated test (blue) RMSE by boosting round when XGBoost uses early stopping, which halts training once the test error stops improving."}
# reproducibility
set.seed(123)
...
> **Warning:** This grid has hundreds of combinations, and a full search over it can take roughly six hours. The loop below is therefore set to `eval = FALSE`. In practice, start with a coarse grid and refine.
...
Gain is usually the most informative, so Figure \@ref(fig:boosting-xgb-importance) plots the top ten features by gain.

```{r boosting-xgb-importance, fig.cap="Top ten features by gain for the final XGBoost model on the Ames housing data."}
...
Local explanations work the same way, except the observations must be one-hot encoded with the same treatment plan before they are explained. Figure \@ref(fig:boosting-xgb-lime) shows the resulting explanations.

```{r boosting-xgb-lime, warning=FALSE, fig.cap="LIME local explanations for two test observations under the final XGBoost model, using the one-hot encoded features."}
...
> **Warning:** PDPs can mislead when the variable of interest is correlated with, or interacts with, the variables being averaged out. ICE curves and accumulated local effects (ALE) plots are useful complements when that is a concern.

To make this concrete, Figure \@ref(fig:boosting-boston-pdp) shows the PDPs for the two most important variables from our `gbm` Boston model, giving each variable's marginal effect on the response after integrating out the others.

```{r boosting-boston-pdp, fig.cap="Partial dependence of predicted median house value on the average number of rooms (rm) and on the percentage of lower-status population (lstat) for the gbm Boston model."}
...
The plots in Figure \@ref(fig:boosting-boston-pdp) confirm the expected directions: predicted median house price rises with the average number of rooms (`rm`) and falls as the percentage of lower-status population (`lstat`) increases.
