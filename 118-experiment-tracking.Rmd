...
The R ecosystem offers several tools that overlap in places. Table \@ref(tab:experiment-tracking-tool-comparison) positions them by primary job.
...
Table: (\#tab:experiment-tracking-tool-comparison) R tools for experiment tracking, model versioning, and deployment, positioned by primary job and capabilities.
...
Because the log is just a `data.frame`, plotting the metric against a parameter is a one-liner. This is the everyday payoff of tracking: the comparison plot comes from records, not from rerunning anything. Figure \@ref(fig:experiment-tracking-rmse-vs-penalty) shows the holdout RMSE traced across the ridge penalties we logged, with the selected run highlighted.

```{r experiment-tracking-rmse-vs-penalty, fig.cap="Holdout RMSE versus ridge penalty, read back from the experiment log. The red point marks the best run.", fig.width=6, fig.height=4}
runs <- read_runs(logfile)
