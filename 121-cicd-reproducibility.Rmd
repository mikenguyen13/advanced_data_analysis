...
Figure \@ref(fig:cicd-reproducibility-gate-distribution) shows the resulting spread of holdout AUC and where the threshold cuts it.

```{r cicd-reproducibility-gate-distribution, fig.cap="Distribution of holdout AUC across 400 bootstrap-trained logistic models. The dashed line is the promotion threshold; models to its left would fail the CI gate.", fig.width=7, fig.height=4.5}
set.seed(7)
...
Having walked the chain end to end, it helps to see all the practices side by side. Table \@ref(tab:cicd-reproducibility-practices) summarizes which dial each one pins, what tool implements it, when it runs, and the specific failure it heads off.
...
Table: (\#tab:cicd-reproducibility-practices) Reproducibility and CI/CD practices side by side, showing which argument of the training function each one pins, the tool that implements it, when it runs, and the failure mode it prevents.
