# Overview of Learning Paradigms {#sec-overview}
...
Within supervised learning, the methods form a few lineages, shown in @fig-overview-map-supervised: smoothers that bend a global curve, instance- and margin-based classifiers, and the tree lineage that grows from a single tree into the most powerful ensembles in the book.

```{mermaid}
%%| label: fig-overview-map-supervised
%%| fig-cap: "The supervised-learning methods and how they relate. The tree lineage grows from a single tree into bagging, random forests, and boosting."
graph LR
...
Unsupervised learning divides by goal, as @fig-overview-map-unsupervised shows: grouping points, compressing them into fewer dimensions, or estimating the density they came from.

```{mermaid}
%%| label: fig-overview-map-unsupervised
%%| fig-cap: "Unsupervised-learning methods, organized by goal: clustering, dimension reduction, and density estimation."
graph TD
...
Deep learning is best read as an evolution, traced in @fig-overview-map-deep: from the multilayer perceptron to convolutional and recurrent networks, then to attention and the Transformer, which underpins BERT and modern large language models, with autoencoders, generative models, and graph networks as parallel branches.

```{mermaid}
%%| label: fig-overview-map-deep
%%| fig-cap: "The deep-learning lineage, from the multilayer perceptron through attention and Transformers to large language models, with generative and graph branches."
graph LR
...
Reinforcement learning has its own progression, shown in @fig-overview-map-rl: from the Markov decision process through dynamic programming and model-free methods to deep reinforcement learning, with bandits, model-based, offline, and multi-agent settings as branches.

```{mermaid}
%%| label: fig-overview-map-rl
%%| fig-cap: "Reinforcement-learning methods, from the Markov decision process to deep RL, with bandit, model-based, offline, and multi-agent branches."
graph TD
...
Finally, the learning paradigms group into four families by the problem they solve, as @fig-overview-map-paradigms shows: where the labels come from, how to learn from few examples, how and where learning happens over time, and how the objective is structured.

```{mermaid}
%%| label: fig-overview-map-paradigms
%%| fig-cap: "The learning paradigms, grouped into four families by the problem they address."
graph TD
...
The two goals are judged by different yardsticks, summarized in @tbl-overview-pred-est.
...
You can predict a sum accurately without knowing its parts. If two predictors move together, the model can tell what their combined effect on $Y$ is, and predict well, while being unsure how to split the credit between them.
:::


A short simulation makes this concrete. We generate two highly correlated predictors whose true coefficients both equal one, fit ordinary least squares on many bootstrap resamples, and watch how the estimated coefficients move. @fig-overview-pred-est-coef shows the result.
...
The individual coefficients in @fig-overview-pred-est-coef range over a wide interval, so any single fit could report a misleading effect size. Their sum, which is what the prediction depends on, barely moves. @tbl-overview-pred-est-sd quantifies this.
...
::: {.callout-warning}
The test set is sacred. If you peek at it while choosing or tuning your model, its error estimate becomes optimistic and stops being an honest forecast of future performance. Touch it only once, after all decisions are made.
:::
...
::: {.callout-tip title="Intuition"}
Learning is search. The hypothesis space says what is on the menu, the quality measure says what "good" means, and the optimization strategy is how we hunt for the best item on the menu.
:::
...
@fig-overview-overfitting shows visually how increasing model flexibility first reduces error, then increases it again as the model begins to chase noise.

```{r}
#| label: fig-overview-overfitting
#| echo: false
#| fig-align: center
#| fig-width: 7
#| fig-height: 4.3
#| fig-cap: "As model flexibility increases, training error keeps falling while test error first decreases and then rises again once the model starts fitting noise. The gap between the two curves is the variance the model pays for extra flexibility."
library(ggplot2)
x <- seq(1, 10, length.out = 200)
train <- 0.85 * exp(-0.45 * x) + 0.04
test <- 0.85 * exp(-0.45 * x) + 0.04 + 0.014 * (x - 2.5)^2
df <- data.frame(
  complexity = rep(x, 2),
  error = c(train, test),
  curve = factor(rep(c("Training error", "Test error"), each = length(x)),
                 levels = c("Training error", "Test error"))
)
best <- x[which.min(test)]
ggplot(df, aes(complexity, error, colour = curve)) +
  geom_line(linewidth = 1.1) +
  geom_vline(xintercept = best, linetype = "dashed", colour = "grey55") +
  annotate("text", x = best, y = max(df$error) * 0.98,
           label = "Best test error", hjust = -0.06, size = 3.4, colour = "grey30") +
  annotate("text", x = 1.6, y = min(test) * 1.4, label = "Underfitting",
           size = 3.4, colour = "grey30") +
  annotate("text", x = 8.8, y = max(test) * 0.9, label = "Overfitting",
           size = 3.4, colour = "grey30") +
  scale_colour_manual(values = c("Training error" = "#1b9e77", "Test error" = "#d95f02")) +
  scale_x_continuous(breaks = NULL) +
  scale_y_continuous(breaks = NULL) +
  labs(x = "Model complexity (flexibility)", y = "Prediction error", colour = NULL) +
...
::: {.callout-tip}
A rough rule of thumb for choosing an evaluation strategy by sample size:
...
: The confusion matrix for binary classification, cross-tabulating predicted against actual labels into true positives (TP), true negatives (TN), false positives (FP), and false negatives (FN). {#tbl-overview-confusion-matrix}
...
$$
\begin{aligned}
\text{Error rate} &= \frac{FP + FN}{N + P} \\[2pt]
\text{Accuracy} &= \frac{TP + TN}{N + P} = 1 - \text{Error rate} \\[2pt]
\text{Sensitivity (recall)} &= \frac{TP}{P} \\[2pt]
\text{Specificity} &= \frac{TN}{TN + FP} \\[2pt]
...
Reach for ROC and precision/recall curves whenever the classes are imbalanced or the costs of false positives and false negatives differ, which is to say, most real problems. A single accuracy number hides exactly the trade-off these curves reveal.
:::
...
The next two subsections zoom in on two of the most important supervised techniques, decision trees (@sec-regression-trees) and random forests (@sec-random-forest), because they illustrate the move from a single interpretable model to a powerful ensemble.
...
::: {.callout-tip title="Intuition"}
Imagine you must transmit the value of $Y$ over a wire. Information gain answers: how many bits, on average, would it save you if both ends of the line already knew $X$? An attribute with high information gain is one whose value resolves a lot of the uncertainty about the class, which is exactly what makes it a good split.
:::
...
::: {.callout-important title="Key idea"}
Averaging many noisy-but-unbiased predictions cancels out their individual errors. The ensemble keeps the trees' low bias while shrinking their variance, which is why a forest beats a lone tree.
:::


The starting point is bagging (@sec-bagging), short for bootstrap aggregating, a general recipe for turning a high-variance method into a low-variance one. The logic is this: if we had many independent realizations of the data, we could fit the model to each and average the results, and averaging many noisy estimates produces a less uncertain one. Concretely, if for each sample $b$ we compute $f^b(x)$, the bagged prediction is
...
::: {.callout-important title="Key idea"}
Adding more trees only attacks the second term. To reduce variance further, you must reduce the correlation $\rho$ between the trees. That single insight is what separates random forests from plain bagging.
:::
...
The defaults for combining and tuning the trees differ between regression and classification, as summarized in @tbl-overview-rf-defaults.
...
::: {.callout-note}
Random forests "cannot overfit" in the sense that adding more trees ($B$ larger) does not increase the model's flexibility, so more trees never hurt generalization. This is different from saying a forest can never be too complex; it is the number of trees, specifically, that is safe to increase.
:::
...
::: {.callout-important title="Takeaways for supervised learning"}
...
The rest of this section develops clustering (@sec-cluster) in some detail, since it makes the central unsupervised idea, defining and exploiting similarity, very concrete.
...
::: {.callout-tip title="Intuition"}
Every clustering method rests on a notion of "close." Change the distance function and you change what counts as a cluster. The algorithm only groups; *you* decide what similarity means.
:::
...
::: {.callout-tip title="Intuition"}
Step 2 says "everyone joins their nearest center," and step 3 says "each center moves to the middle of its new members." Repeating these two moves slides the centers into the dense regions of the data until they stop moving.
:::
...
::: {.callout-warning}
Because k-means only finds a local optimum and starts from random seeds, two runs on the same data can give different answers. Run it several times with different seeds and keep the best (lowest-SSE) result rather than trusting a single run.
:::
...
::: {.callout-tip title="When to use this"}
Semi-supervised methods earn their keep when unlabeled data are abundant but labels are scarce or costly, as in medical imaging, speech, or any domain where expert annotation is the bottleneck.
:::
