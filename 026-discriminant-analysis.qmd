...
### Deriving the discriminant functions from Bayes' rule
...
$$
\log \pi_k + \log f_k(x) = -\tfrac{1}{2}\log|\Sigma_k| - \tfrac{1}{2}(x-\mu_k)^T \Sigma_k^{-1}(x-\mu_k) + \log\pi_k + \text{const}.
$$ {#eq-discriminant-analysis-qda-derivation}

This is exactly the QDA discriminant $\delta_k(x)$ stated above: the quadratic form in $x$ is what gives quadratic boundaries.
...
$$
\delta_k(x) = x^T \Sigma^{-1}\mu_k - \tfrac{1}{2}\mu_k^T \Sigma^{-1}\mu_k + \log\pi_k,
$$ {#eq-discriminant-analysis-lda-derivation}
...
The following short simulation confirms that the simplified linear discriminant @eq-discriminant-analysis-lda-derivation ranks classes identically to the full Gaussian log-posterior, that is, the dropped class-independent terms truly do not change the $\arg\max$.

```{r}
#| label: lda-discriminant-check
set.seed(1)
p <- 3; K <- 3
...
::: {.callout-note title="The decision boundary is linear"}
Between any two classes $k$ and $l$, the LDA boundary is the set where $\delta_k(x) = \delta_l(x)$. Subtracting the two linear discriminants gives
$$
\log\frac{\pi_k}{\pi_l} - \tfrac{1}{2}(\mu_k + \mu_l)^T \Sigma^{-1}(\mu_k - \mu_l) + x^T \Sigma^{-1}(\mu_k - \mu_l) = 0,
$$
an affine equation in $x$, hence a hyperplane. For QDA the same difference retains a term $-\tfrac{1}{2}x^T(\Sigma_k^{-1} - \Sigma_l^{-1})x$, giving a quadric surface.
:::

### Parameter estimates as maximum likelihood
...
\hat\mu_k = \frac{1}{n_k}\sum_{g_i = k} x_i, \qquad
\hat\Sigma_k = \frac{1}{n_k}\sum_{g_i = k}(x_i - \hat\mu_k)(x_i - \hat\mu_k)^T,
$$
...
$$ {#eq-discriminant-analysis-pooled-cov}
...
#### Deriving the eigenproblem {.unnumbered}
...
$$
\mathcal{R}(a) = \frac{a^T \mathbf{B}\, a}{a^T \mathbf{W}\, a}.
$$ {#eq-discriminant-analysis-rayleigh}
...
$$
\mathbf{B}\, a = \lambda\, \mathbf{W}\, a \quad\Longleftrightarrow\quad \mathbf{W}^{-1}\mathbf{B}\, a = \lambda\, a,
$$ {#eq-discriminant-analysis-geneig}
...
::: {.callout-tip title="Whitening makes this a symmetric problem"}
...
#### Why optimal scoring recovers LDA {.unnumbered}
...
$$
\min_{\theta,\,\beta}\; \| \mathbf{Y}\theta - \mathbf{X}\beta \|^2
\quad\text{subject to}\quad
\tfrac{1}{n}\,\theta^T \mathbf{Y}^T \mathbf{Y}\,\theta = 1,
$$ {#eq-discriminant-analysis-optscore}
...
$$
\min_\theta\; \| \mathbf{Y}\theta - \mathbf{P}\mathbf{Y}\theta\|^2
= \min_\theta\; \theta^T \mathbf{Y}^T(\mathbf{I} - \mathbf{P})\mathbf{Y}\,\theta,
$$
...
::: {.callout-warning}
Fitting these mixtures is harder than fitting plain LDA, because the optimization is non-convex and can get stuck. The parameters are typically estimated with the expectation-maximization (E-M) algorithm [@hastie2001a 12.7] or a fully Bayesian implementation. Expect to try several starting values.
:::

#### The EM updates for MDA {.unnumbered}
...
$$
w_{ir} = \frac{q_{kr}\,\phi(x_i;\, \mu_{kr},\, \Sigma)}{\sum_{s=1}^{R_k} q_{ks}\,\phi(x_i;\, \mu_{ks},\, \Sigma)} .
$$ {#eq-discriminant-analysis-mda-estep}
...
$$
\hat q_{kr} = \frac{\sum_{i} w_{ir}}{n_k},
\qquad
\hat\mu_{kr} = \frac{\sum_{i} w_{ir}\, x_i}{\sum_{i} w_{ir}},
$$
...
$$
\hat\Sigma = \frac{1}{n}\sum_{k=1}^K \sum_{r=1}^{R_k} \sum_{i:\, g_i=k} w_{ir}\,(x_i - \hat\mu_{kr})(x_i - \hat\mu_{kr})^T .
$$ {#eq-discriminant-analysis-mda-mstep}
...
#### Why soft-thresholding is the right operator {.unnumbered}
...
$$
d^\star = \operatorname{sign}(\hat d)\,(|\hat d| - \Delta)_+ .
$$ {#eq-discriminant-analysis-soft-threshold}
